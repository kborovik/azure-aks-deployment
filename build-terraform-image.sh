...
TERRAFORM_VERSION="$(grep -e '^ARG TERRAFORM_VERSION' docker/terraform/Dockerfile | cut -d "=" -f 2)"
...
docker build --tag="${DOCKER_TAG}:${TERRAFORM_VERSION}" --rm --file="docker/terraform/Dockerfile" .
